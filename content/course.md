...
这一阶段默认已经具备编程与计算机系统基础，重点强化 CUDA 工程会直接用到的能力。
...
这些内容已拆成左侧可独立学习的教程页，不只列名词：

* 内存与对象生命周期：栈、堆、指针、引用与 RAII；
* 编译、链接与库：声明、定义、目标文件、静态库与动态库；
* Cache、局部性与内存对齐：连续访问、Cache Line、对齐与 `restrict` 思想；
* 模板与编译期计算：类型模板、非类型模板参数、`constexpr` 与 `static_assert`；
* CMake 工程组织：Target、依赖传播、库、测试与独立构建目录；
* GDB 与 AddressSanitizer：调用栈、断点、越界和释放后使用。
